...
- **New review surface for any future PE-bearing `talent_infra_modules/*/deploy.ps1`:** verify the script implements the four-step stale-`privateDnsZoneGroup` self-heal pattern (detect → surface in plan summary → act gated → optional same-RG empty-and-unlinked orphan-zone cleanup) and exposes `[switch]$FixStaleDnsZoneGroup` with `-Force` implication. Reference implementation: `talent_infra_modules/01-postgresql/deploy.ps1` Sections 6c / 7b / 7c (2026-05-22T18:00:00Z). Normative spec: `.squad/decisions.md` 2026-05-22T18:00:00Z. Reusable template: `.squad/skills/azure-pe-dns-zone-group-self-heal/SKILL.md`.
- **Hazard-checklist addition for future toolkit reviews:** add row for "Stale PE `privateDnsZoneGroup` self-heal — script-only fix, Bicep cannot recover from `UpdatingPrivateDnsZoneIdOnPrivateDnsZoneConfigNotAllowed`. Check the switch exists, the destructive path is gated, the orphan-zone cleanup respects same-RG + `rsCount -le 1 -and linkCount -eq 0` guards, and the README documents the switch in the Inputs table." Update `/memories/repo/talentiq-azd-deploy.md` next pass.

## Cross-agent note — 2026-05-22T22:30:00Z (Scribe)
- **Sweep assignment.** Two new normative patterns from Bishop (decisions.md `2026-05-22T22:20:00Z`) require sweeping the four sibling `talent_infra_modules/{00,02,03,04}/deploy.ps1` files. Pattern A (DNS-RP lag): in orphan-zone cleanup, gate on `az network private-dns record-set list --query "[?type!='Microsoft.Network/privateDnsZones/SOA']"` rather than `numberOfVirtualNetworkLinks` or `link vnet list` (both lag the RP). Best-effort idempotent named-delete of links is authoritative; treat zone-delete failure as non-fatal. Pattern B (JSON capture): replace any `Invoke-Native { az deployment group create ... 2>&1 }` with stderr redirected to `$scriptDir/.deploy-logs/{stamp}-bicep-stderr.txt`; capture stdout only; force `-o json`; validate non-empty before `ConvertFrom-Json`; on parse failure dump raw stdout to disk (never echo — may contain secrets/IDs). Section-8 shape in `01-postgresql/deploy.ps1` is the canonical replacement. **Model directive (2026-05-22):** all future squad spawns (including Scribe/Ralph) use `claude-opus-4.6-1m` per `.squad/config.json` `defaultModel`.
