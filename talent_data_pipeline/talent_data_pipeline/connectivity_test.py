...
def run_connectivity_test() -> bool:
    ...
    # 4. DiskANN — optional, not fatal. Check both extension names.
    print("[4/5] DiskANN extension (optional)")
    diskann_ver = None
    # Check if either extension is already installed
    cur.execute(
        "SELECT extname, extversion FROM pg_extension WHERE extname IN ('vectorscale', 'pg_diskann');"
    )
    existing = cur.fetchone()
    if existing:
        diskann_ver = existing[1]
        print(f"  Extension '{existing[0]}' v{diskann_ver} — OK")
    else:
        # Try creating vectorscale first, then pg_diskann
        for ext in ("vectorscale", "pg_diskann"):
            diskann_ver = _ensure_extension(cur, conn, ext)
            if diskann_ver:
                break
    if not diskann_ver:
        print("  INFO: DiskANN not available — will use HNSW indexes instead")
